...
class MangaKakalot extends paperback_extensions_common_1.Source {
    ...
    async getMangaDetails(mangaId) {
        const request = createRequestObject({ url: `${this.baseUrl}/manga/${mangaId}`, method: 'GET' });
        ...
        const status = $('.manga-info-text li:contains("Status")').text().includes('Ongoing') ? paperback_extensions_common_1.MangaStatus.ONGOING : paperback_extensions_common_1.MangaStatus.COMPLETED;
        const tags = [];
        $('.manga-info-text .manga-info-genre a').each((_, el) => {
            const label = $(el).text().trim();
            tags.push(createTag({ id: label.toLowerCase(), label }));
        });
        const tagSection = createTagSection({ id: 'genres', label: 'Genres', tags });
        return createManga({ id: mangaId, titles: [title], image, author, desc, status, tags: [tagSection] });
    }
    async getChapters(mangaId) {
        ...
        $('.chapter-list .row').each((index, el) => {
            ...
            chapters.push(createChapter({ id, mangaId, name, chapNum, langCode: paperback_extensions_common_1.LanguageCode.ENGLISH }));
        });
        return chapters;
    }
    async getChapterDetails(mangaId, chapterId) {
        const request = createRequestObject({ url: `${this.baseUrl}/chapter/${mangaId}/${chapterId}`, method: 'GET' });
        const response = await this.requestManager.schedule(request, 1);
        const $ = this.cheerio.load(response.data);
        const pages = [];
    ...
    async getSearchResults(query, metadata) {
        ...
        const request = createRequestObject({ url: `${this.baseUrl}/search/story/${searchTerm}`, method: 'GET' });
        const response = await this.requestManager.schedule(request, 1);
        const $ = this.cheerio.load(response.data);
        const tiles = [];
        $('.story-item').each((_, el) => {
            ...
            if (id && title)
                tiles.push(createMangaTile({ id, title: createIconText({ text: title }), image }));
        });
